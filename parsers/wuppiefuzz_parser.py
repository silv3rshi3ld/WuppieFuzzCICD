    ...
    def get_bug_information(self):
        ...
        for row in self.cursor.fetchall():
            bug_info = {
                ...
                "request": self.decode_if_bytes(row[3]),
                "response": self.decode_if_bytes(row[4])
            }
            print(f"Bug: {bug_info}")
...
if __name__ == '__main__':
    ...
    try:
        with WuppieFuzzParser(zip_path) as parser:
            ...
            output_path = os.path.join(os.path.dirname(zip_path), 'wuppiefuzz_report.json')
            with open(output_path, 'w') as f:
                json.dump(report, f, indent=2)
            print(f"Wuppiefuzz report saved to {output_path}")
